...
cspsum_raw = (csp.imag).sum(axis=0)
csmsum_raw = (csm.imag).sum(axis=0)
stdp1 = np.sqrt(varp.imag)
stdn1 = np.sqrt(varn.imag)

# cspsum_smooth1 = savgol(cspsum_raw, WINLEN, POLYORD)
# csmsum_smooth1 = savgol(csmsum_raw, WINLEN, POLYORD)

fname_suffix = f"{args.n:02d}_{args.l:03d}_{args.lp:03d}"
if args.t != 0:
    fname_suffix += f"_{args.t:03d}"
fname_suffix += "_smooth.npy"
csp = np.load(f"{writedir}/csp_data_{fname_suffix}")
csm = np.load(f"{writedir}/csm_data_{fname_suffix}")
varp = np.load(f"{writedir}/variance_p_{fname_suffix}")
varn = np.load(f"{writedir}/variance_n_{fname_suffix}")
...
stdp2 = np.sqrt(varp.imag)
stdn2 = np.sqrt(varn.imag)
cspsum_smooth2 = cspsum
csmsum_smooth2 = csmsum
...
axs.flatten()[0].fill_between(freq_win_p, cspsum-stdp2, cspsum+stdp2,
                              color='red', alpha=0.4, label="1$\sigma$ - smooth data")
axs.flatten()[0].fill_between(freq_win_p, cspsum-stdp1, cspsum+stdp1,
                              color='gray', alpha=0.4, label="1$\sigma$ - raw data")
axs.flatten()[0].plot(freq_win_p, cspsum_raw, 'k', alpha=0.7, label="Raw spectrum")
axs.flatten()[0].plot(freq_win_p, cspsum_smooth2, 'r', label="Smoothened spectrum")
axs.flatten()[0].set_title(f" n = {args.n}; $\ell_1-\ell_2$ = {args.l}$-${args.lp}; m>0")
axs.flatten()[0].set_xlabel(f"Frequency in $\mu$Hz")
axs.flatten()[0].legend()

axs.flatten()[1].fill_between(freq_win_p, csmsum-stdn2, csmsum+stdn2,
                              color='red', alpha=0.4, label="1$\sigma$ - smooth data")
axs.flatten()[1].fill_between(freq_win_n, csmsum-stdn1, csmsum+stdn1,
                              color='gray', alpha=0.4, label="1$\sigma$ - raw data")
axs.flatten()[1].plot(freq_win_n, csmsum_raw, 'k', alpha=0.7, label="Raw spectrum")
axs.flatten()[1].plot(freq_win_n, csmsum_smooth2, 'r', label="Smoothened spectrum")
